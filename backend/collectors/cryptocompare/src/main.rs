...
impl CollectorHelper for Urly {
    fn create_url(&self, cryptocurrencies: &[SourceCryptoMapping]) -> String {
        let param_string = cryptocurrencies
            ...
            .collect::<Vec<_>>()
            .join(",");
        format!("https://min-api.cryptocompare.com/data/pricemulti?fsyms={param_string}&tsyms=USD")
    }
...
async fn main() {
    ...
    csb_collector_common::run(connection_string, "CryptoCompare", Box::new(Urly {}))
        .await
        .unwrap()
}
